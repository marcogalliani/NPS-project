...
perm_t.uni.test <- function(data1, data2, test_stat, B, seed, cl){
  ...
  clusterExport(cl=cl, list('data1', 'data2', "n1","n","test_stat"), envir = environment())
  
  test_stat_eval <- function(permutation){
    data.pooled <- c(data1,data2)
    
    perm.pooled.data <- data.pooled[permutation]
    
    data1.perm <- perm.pooled.data[1:n1]
    data2.perm <- perm.pooled.data[(n1+1):n]
...
boot.estimation <- function(data, t_stat, alpha, B, seed, cl){
  ...
  test_stat_eval <- function(boot.indices){
    return(t_stat(data[boot.indices,]))
  }
...
boot.uni.estimation <- function(data, t_stat, alpha, B, seed, cl){
  ...
  #(4) Confidence intervals (-> reverse percentile intervals)
  left.quantile <- quantile(estimation_results$boot_distr.T_stat, probs=alpha/2) 
  right.quantile  <- quantile(estimation_results$boot_distr.T_stat, probs=1-alpha/2) 
